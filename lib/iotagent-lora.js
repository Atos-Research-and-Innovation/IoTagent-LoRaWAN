...
function loadConfigurationFromAppserver (appServer, callback) {
    if (!appServer.getIotaConfiguration()) {
        console.log('no hayyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy');
        return iotAgentLib.getConfiguration(appServer.getAppId(), '', callback);
    } else {
        var conf = appServer.getIotaConfiguration();
        console.log('resssssssssssssssssssssssssssssssssssssssssss');
 ...
 */
function messageHandler (appServer, deviceId, message) {
    var errorMessage;
    if (!appServer) {
    ...
    if (!deviceObject) {
        ...
        async.waterfall([
            ...
            async.apply(registerDeviceFromConfiguration, deviceId)
...
function registerApplicationServer (appServerConf, iotaConfiguration, callback) {
    ...
    if (!appServerConf.lorawan.application_server.provider) {
        error = 'Provider for application server is required. Supported values: TTN';
        winston.error(error);
        return callback(error);
    }

    if (!appServerConf.lorawan.app_eui || !appServerConf.lorawan.application_id) {
        error = 'Missing mandatory configuration attributes for lorawan';
        winston.error(error);
        return callback(error);
    }
    ...
    for (var app in loraApps) {
        if (loraApps[app] instanceof appService.AbstractAppService) {
            if (loraApps[app].getAppId() === appServerConf.lorawan.app_eui) {
                ...
                console.log(JSON.stringify(loraApps[app].getIotaConfiguration()));
...
function registerDevice (device, callback) {
    ...
    registerApplicationServer(lorawanConf, null, function (err, appServer) {
        ...
        appServer.addDevice(device.id, device);
 ...
 * @param      {Function}  callback       The callback
 */
function registerDeviceFromConfiguration (deviceId, configuration, callback) {
    var newDevice = {};
    newDevice = {
        id: deviceId,
...
function loadTypesFromConfig (callback) {
    ...
    if (config.getConfig().iota.types) {
        ...
        for (var type in config.getConfig().iota.types) {
            if (config.getConfig().iota.types[type].internalAttributes && config.getConfig().iota.types[type].internalAttributes.lorawan) {
        ...
        async.eachSeries(arrayTypes,
            registerConfigType, function (err) {
                if (err) {

...
function findOrCreate(deviceObject, group, callback) {
    ...
    iotAgentLib.getDevice(deviceObject.id, group.service, group.subservice, function(error, device) {
        if (!error && device) {
            ...
            const internalAttrs = [];
            var devEui = findLorawanInternalAttribute('dev_eui', deviceObject.internalAttributes);
            var dataModel = findLorawanInternalAttribute('data_model', group.internalAttributes);
            ...
            if (devEui) {
                internalAttrs[0].lorawan.dev_eui = devEui;
            }

            if (dataModel) {
                internalAttrs[0].lorawan.data_model = dataModel;
            }

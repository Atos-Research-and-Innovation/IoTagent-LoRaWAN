...
function loadConfigurationFromAppserver(appServer, callback) {
    if (!appServer.getIotaConfiguration()) {
        ...
        if (!conf.resource) {
            ...
            return iotAgentLib.getConfiguration(
                appServer.getAppId(),
                '',
                callback
            );
    ...
    if (!deviceObject) {
        ...
        async.waterfall(
            [
                ...
                async.apply(
                    registerDeviceFromConfiguration,
                    deviceId,
                    deviceEui
                ),
            ],
            function(error, device) {
                if (error) {
                    ...
                    if (
                        ngsiMessage &&
                        ngsiMessage instanceof Array &&
                        ngsiMessage.length > 0
                    ) {
                        iotAgentLib.update(
                            device.name,
                            device.type,
                            ...
                            function(iotaError) {
                                if (iotaError) {
                                    ...
                                    winston.error(
                                        errorMessage,
                                        JSON.stringify(iotaError)
                                    );
                                } else {
                                    winston.info(
                                        'Observations sent to CB successfully for device ',
                                        deviceId
                                    );
                                }
                            }
        ...
        iotAgentLib.getDevice(
            ...
            function(error, device) {
                if (error) {
                    ...
                    errorMessage =
                        "Couldn't find device data for DeviceId " + deviceId;
                    winston.error(errorMessage);
                }
            }
        );
...
function registerApplicationServer(appServerConf, iotaConfiguration, callback) {
    ...
    winston.info(
        'Registering Application Server:%s',
        JSON.stringify(appServerConf)
    );

    if (!appServerConf.lorawan) {
        error =
            'lorawan attribute must be specified inside internal_attributes';
        winston.error(error);
    ...
    if (!appServerConf.lorawan.application_server) {
        error =
            'lorawan.application_server attribute must be specified inside internal_attributes';
        winston.error(error);
        return callback(error);
    }
    ...
    if (!appServerConf.lorawan.application_server.provider) {
        error =
            'Provider for application server is required. Supported values: TTN and loraserver.io';
        winston.error(error);
        return callback(error);
    }
    ...
    if (!appServerConf.lorawan.app_eui) {
        error =
            'Missing mandatory configuration attributes for lorawan: app_eui';
        winston.error(error);
        return callback(error);
    }
    ...
    if (!appServerConf.lorawan.application_id) {
        error =
            'Missing mandatory configuration attributes for lorawan: application_id';
        winston.error(error);
        return callback(error);
    }
        ...
        if (loraApps[app] instanceof appService.AbstractAppService) {
            if (loraApps[app].getAppId() === appServerConf.lorawan.app_eui) {
                ...
                if (iotaConfiguration && loraApps[app].getIotaConfiguration()) {
                    error =
                        'Could not assign a new type or service to the LoRaWAN Application';
                    winston.error(error);
                    return callback(error);
                } else {
    ...
    if (appServerConf.lorawan.application_server.provider === 'TTN') {
        ...
        error =
            'Unsupported provider for application server. Supported values: TTN and loraserver.io';
        winston.error(error);
        return callback(error);
    }
...
function registerConfiguration(configuration, callback) {
    ...
    winston.info(
        'Configuration provisioning:%s',
        JSON.stringify(configuration)
    );

    if (!configuration.internalAttributes) {
        error =
            'internal_attributes is mandatory to define specific agent configuration';
        winston.error(error);
        error = { message: error };
        return callback(error);
    } else {
        ...
        registerApplicationServer(lorawanConf, configuration, function(
            err,
            ttnApp
        ) {
            if (err) {
...
function removeDevice(device, callback) {
    ...
    for (var app in loraApps) {
        if (loraApps[app] instanceof appService.AbstractAppService) {
            if (loraApps[app].getAppId() === lorawanConf.lorawan.app_eui) {
                loraApps[app].removeDevice(
                    device.id,
                    lorawanConf.lorawan.dev_eui,
                    device
                );
                break;
            }
 ...
 * @param      {Function}  callback       The callback
 */
function registerDeviceFromConfiguration(
    deviceId,
    deviceEUI,
    configuration,
    callback
...
function loadTypesFromConfig(callback) {
    ...
    if (config.getConfig().iota.types) {
        ...
        async.eachSeries(arrayTypes, registerConfigType, function(err) {
            if (err) {
                winston.error(
                    'Error loading services from configuration file',
                    err
                );
                return callback(err);
            } else {
...
function loadServices(callback) {
    ...
    confService.list(null, 100, 0, function(err, services) {
        ...
        if (services.count > 0 && services.services.length > 0) {
            async.eachSeries(services.services, registerConfiguration, function(
                err
            ) {
                if (err) {
...
function loadDevices(callback) {
    ...
    iotAgentLib.listDevices(
        ...
        function(err, devices) {
            ...
            if (devices) {
                async.eachSeries(devices.devices, registerDevice, function(
                    err
                ) {
                    if (err) {
...
function start(newConfig, callback) {
    ...
    iotAgentLib.activate(config.getConfig().iota, function(error) {
        if (error) {
            ...
            async.waterfall(
                [loadTypesFromConfig, loadServices, loadDevices],
                function(error) {
                    if (error) {
                        winston.error(error);
                        return callback(error);

...
class MqttClient {
     ...
     */
    constructor (host, username, password, options, listener) {
        if (!host || !listener) {
    ...
    start (callback) {
        ...
        if (options.ca && options.cert && options.key) {
            host = options['protocol'] + '://' + this.host;
            this.options.ca = Buffer.from(this.options.ca);
            this.options.cert = Buffer.from(this.options.cert);
            this.options.key = Buffer.from(this.options.key);
        } else {
            host = 'mqtt://' + this.host;
        }

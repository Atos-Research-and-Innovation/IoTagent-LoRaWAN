...
const CAYENNELPP_MAX_CHANNEL = 99;
            ...
            if (validateCayenneLppChannel(channel)) {
                ...
                case LPP_GPS:
                    if (cursor + LPP_GPS_SIZE > buffer.length) {
                        ...
                        var latitude = readUInt24BE(buffer, cursor) / 1000.0;
                        cursor += 3;
                        var longitude = readUInt24BE(buffer, cursor) / 1000.0;
                        cursor += 3;
                        var altitude = readUInt24BE(buffer, cursor) / 100.0;
                        cursor += 3;
...
/**
 * It reads an unigned integer represented using 24 bits
 *

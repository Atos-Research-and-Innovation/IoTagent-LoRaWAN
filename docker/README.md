...
should be increased. By default, we recommend, that the IoT Agent for LoRaWAN server in high performance scenarios, the
...
To set the ulimits in your container, you will need to run IoT Agent for LoRaWAN Docker containers with the following
...
--name iotagent -d fiware/iotagent-lorawan
```
...
> Note: The --ulimit flags only work on Docker 1.6 or later. Nevertheless, you have to "request" more resources to
> Kubernetes (i.e. multiple cores), which might be more difficult for [Kubernetes](https://kubernetes.io/) to schedule
> than a few different containers requesting one core (or less...) each (which it can, in turn, schedule on multiple
> nodes, and not necessarily look for one node with enough available cores).

If you want to get more details about the configuration of the system and node.js for high performance scenarios, please
refer to the [Installation Guide](https://fiware-lorawan.readthedocs.io/en/latest/installationguide/index.html).
...
details are retrieved from a database. The default name of the IoT Agent database is `iotagentlora`. Database access can be
